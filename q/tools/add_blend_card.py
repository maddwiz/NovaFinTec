#!/usr/bin/env python3
# tools/add_blend_card.py  (oscillator blend card)
...
CARD_START="<!--BLEND_OSC_CARD_START-->"
CARD_END="<!--BLEND_OSC_CARD_END-->"
...
if __name__ == "__main__":
    s = RUNS / "blend_osc_main_summary.json"
    c = RUNS / "blend_osc_main.csv"
    if not s.exists() or not c.exists():
        raise SystemExit("Run blend_osc_with_main.py first.")
    meta = _json.loads(s.read_text())
    df = pd.read_csv(c, parse_dates=["DATE"])
    png = RUNS / "blend_osc_main.png"
    try:
        import matplotlib.pyplot as plt
        plt.figure()
        plt.plot(df["DATE"], df["eq"])
        plt.title(f"Blended Equity: Main × Osc (alpha={meta.get('alpha',0):.2f})")
...
<section style="border:1px solid #ddd;padding:12px;margin:16px 0;border-radius:8px">
  <h2 style="margin:0 0 8px 0">Blended Portfolio: Main × Oscillator</h2>
  <p style="color:#666;margin:4px 0 8px 0">Static blend chosen by out-of-sample Sharpe.</p>
  ...
  {"<img src='runs_plus/blend_osc_main.png' style='max-width:100%'>" if png else ""}
</section>
{CARD_END}"""
    for f in FILES:
        if not f.exists():
        ...
        print(f"✅ Upserted Blend(Main×Osc) card in", f.name)
